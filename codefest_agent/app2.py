...
<style>
    ...
    .stat-number { font-size: 2rem; font-weight: 700; color: #4FC3F7; }
...
def generate_pdf(report):
    ...
    story.append(Paragraph(f"Sources Searched: {s['sources_searched']}", styles["Normal"]))
    ...
    story.append(Paragraph(f"With Rating: {q['records_with_rating']}", styles["Normal"]))
    story.append(Paragraph(f"With Hours: {q['records_with_hours']}", styles["Normal"]))
    story.append(Spacer(1, 10))

    story.append(Paragraph("Business Results", styles["Heading2"]))
    for i, b in enumerate(report["businesses"], 1):
        story.append(Paragraph(f"{i}. {b.get('business_name','N/A')}", styles["Heading3"]))
        ...
        story.append(Paragraph(f"Hours   : {b.get('working_hours') or 'N/A'}", styles["Normal"]))
        story.append(Paragraph(f"Trust Score: {b.get('trust_score','N/A')}/10", styles["Normal"]))
        if b.get("conflict_flags"):
            story.append(Paragraph(f"⚠ Conflicts: {b['conflict_flags']}", styles["Normal"]))
        story.append(Spacer(1, 8))

    story.append(Spacer(1, 20))
    story.append(Paragraph(
        "AI Business Research Agent | Chettinad CodeFest 2026 | Achyuta K & Bhuvaneshwari | SASTRA University",
        styles["Normal"]
    ))
...
if search_btn and query:
    ...
    with c1:
        st.markdown(f'<div class="stat-box"><div class="stat-number">{s["businesses_found"]}</div><div class="stat-label">Found</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="stat-box"><div class="stat-number">{s["businesses_verified"]}</div><div class="stat-label">Verified</div></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="stat-box"><div class="stat-number">{s["duplicates_removed"]}</div><div class="stat-label">Duplicates Removed</div></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(f'<div class="stat-box"><div class="stat-number">{s["sources_searched"]}</div><div class="stat-label">Sources Searched</div></div>', unsafe_allow_html=True)
    with c5:
        st.markdown(f'<div class="stat-box"><div class="stat-number">{q["records_with_phone"]}</div><div class="stat-label">Have Phone</div></div>', unsafe_allow_html=True)
    ...
    with left:
        ...
        for b in businesses:
            ...
            <div class="business-card">
                ...
                <div class="info-row">Trust Score: <span class="{score_class}">{score}/10</span></div>
                {conflict_html}
